...
Visualizaciones interactivas con estética tipo Flourish + Explorador de Tablas
- Diseño limpio y moderno
- Paleta de colores profesional
- Tipografía elegante
- Espaciado generoso
- Visualización de tablas con JOINs automáticos
...
# ============================================================================
# DEFINICIÓN DE TABLAS Y QUERIES CON JOINS
# ============================================================================

# Diccionario con todas las tablas y sus queries optimizadas con JOINs
TABLE_QUERIES = {
    'convocatoria': {
        'name': '📋 Convocatorias',
        'query': """
            SELECT 
                ...
                c.estado,
                p.cant_residentes_plan as residentes_planificados,
    ...
    'menu': {
        'name': '🖥️ Asignaciones de Dispositivos',
        'query': """
            SELECT 
                m.id_menu,
                dp.nombre || ' ' || dp.apellido as agente,
    ...
    'capacitaciones': {
        ...
        'query': """
            SELECT 
                ...
                GROUP_CONCAT(DISTINCT disp.nombre_dispositivo, ', ') as dispositivos,
            ...
            LEFT JOIN capacitaciones_dispositivos cd ON cap.id_cap = cd.id_cap
    ...
    'capacitaciones_participantes': {
        'name': '👥 Participantes de Capacitaciones',
        'query': """
            SELECT 
                cp.id_participante,
                cap.tema as capacitacion,
                d.fecha as fecha_capacitacion,
                dp.nombre || ' ' || dp.apellido as participante,
                coord.nombre || ' ' || coord.apellido as coordinador,
                CASE WHEN cp.asistio = 1 THEN '✓ Asistió' ELSE '✗ No asistió' END as asistencia,
                CASE 
                    WHEN cp.aprobado = 1 THEN '✓ Aprobado'
                    WHEN cp.aprobado = 0 THEN '✗ No aprobado'
                    ELSE 'Pendiente'
                END as resultado,
                cp.calificacion,
                cp.fecha_certificado,
                cp.observaciones
            FROM capacitaciones_participantes cp
            JOIN capacitaciones cap ON cp.id_cap = cap.id_cap
            JOIN datos_personales dp ON cp.id_agente = dp.id_agente
            JOIN dias d ON cap.id_dia = d.id_dia
            JOIN datos_personales coord ON cap.coordinador_cap = coord.id_agente
            ORDER BY d.fecha DESC, dp.apellido
            LIMIT 500
        """
    },
    'dispositivos': {
        ...
        'query': """
            SELECT 
                ...
                COUNT(DISTINCT m.id_agente) as agentes_capacitados,
                COUNT(m.id_menu) as total_asignaciones,
    ...
    'turnos': {
        ...
        'query': """
            SELECT 
                ...
                CASE WHEN t.solo_fines_semana = 1 THEN 'Sí' ELSE 'No' END as solo_fines_semana,
                CASE WHEN t.solo_semana = 1 THEN 'Sí' ELSE 'No' END as solo_semana,
                CASE WHEN t.activo = 1 THEN '✓ Activo' ELSE '✗ Inactivo' END as estado,
                COUNT(DISTINCT c.id_convocatoria) as convocatorias_totales,
                t.turno_notas
            FROM turnos t
            LEFT JOIN convocatoria c ON t.id_turno = c.id_turno
            GROUP BY t.id_turno
    ...
    'planificacion': {
        ...
        'query': """
            SELECT 
                ...
                p.hora_fin,
                p.cant_horas,
                CASE WHEN p.usa_horario_custom = 1 THEN '✓ Custom' ELSE 'Catálogo' END as tipo_horario,
                p.cant_residentes_plan as residentes,
                p.cant_visit as visitantes,
                COUNT(DISTINCT c.id_convocatoria) as convocatorias_asignadas,
                p.plani_notas
    ...
    'descansos': {
        ...
        'query': """
            SELECT 
                ...
                des.observaciones
            FROM descansos des
            JOIN datos_personales dp ON des.id_agente = dp.id_agente
            ORDER BY des.fecha_solicitud DESC
            LIMIT 500
        """
    },
    'disponibilidad': {
        ...
        'query': """
            SELECT 
                ...
                disp.estado,
                CASE 
                    WHEN disp.prioridad = 1 THEN '⭐ Alta'
                    WHEN disp.prioridad = 2 THEN '• Normal'
                    WHEN disp.prioridad = 3 THEN '↓ Baja'
                END as prioridad,
                disp.fecha_declaracion
            FROM disponibilidad disp
            JOIN datos_personales dp ON disp.id_agente = dp.id_agente
            JOIN turnos t ON disp.id_turno = t.id_turno
    ...
    'convocatoria_historial': {
        ...
        'query': """
            SELECT 
                h.id_hist,
                c.fecha_convocatoria,
                ant.nombre || ' ' || ant.apellido as agente_anterior,
                nue.nombre || ' ' || nue.apellido as agente_nuevo,
                h.tipo_cambio,
                h.motivo,
                h.fecha_cambio,
                h.usuario_responsable
            FROM convocatoria_historial h
            JOIN convocatoria c ON h.id_convocatoria = c.id_convocatoria
            JOIN datos_personales ant ON h.id_agente_anterior = ant.id_agente
            JOIN datos_personales nue ON h.id_agente_nuevo = nue.id_agente
            ORDER BY h.fecha_cambio DESC
            LIMIT 300
        """
    },
    'cambio_transaccion': {
        'name': '🔄 Transacciones de Cambio',
        'query': """
            SELECT 
                ct.id_transaccion,
                dp.nombre || ' ' || dp.apellido as agente_iniciador,
                ct.tipo_transaccion,
                ct.estado,
                ct.fecha_solicitud,
                ct.fecha_validacion,
                ct.fecha_aprobacion,
                ct.fecha_ejecucion,
                ct.usuario_aprobador,
                ct.motivo_rechazo,
                ct.observaciones
            FROM cambio_transaccion ct
            JOIN datos_personales dp ON ct.agente_iniciador = dp.id_agente
    ...
    'system_errors': {
        ...
        'query': """
            SELECT 
                ...
                e.error_message,
                e.severity,
                CASE WHEN e.resolved = 1 THEN '✓ Resuelto' ELSE '⏳ Pendiente' END as estado,
                dp.nombre || ' ' || dp.apellido as agente_afectado,
                e.user_action,
                e.resolution_date,
                e.resolved_by
    ...
    'dias': {
        ...
        'query': """
            SELECT 
                ...
                mes,
                semana,
                CASE WHEN es_feriado = 1 THEN '🎉 ' || nombre_feriado ELSE 'Día normal' END as feriado
            FROM dias
            ORDER BY fecha DESC
            LIMIT 365
        """
...
# ============================================================================
# FUNCIONES DE DATOS (código anterior sin cambios)
# ============================================================================
...
def get_table_data(table_key):
    """Obtener datos de una tabla específica con JOINs"""
    try:
        conn = get_connection()
        query = TABLE_QUERIES[table_key]['query']
...
# [AQUÍ IRÍAN TODAS LAS FUNCIONES ANTERIORES DE DATOS Y VISUALIZACIÓN]
...
# ============================================================================
# TEMPLATE FLOURISH PARA GRÁFICOS (código anterior sin cambios)
# ============================================================================
...
# [AQUÍ IRÍAN TODAS LAS FUNCIONES DE VISUALIZACIÓN ANTERIORES]
# create_inasistencias_charts, create_saldos_charts, etc.
# Por brevedad las omito pero deben estar presentes
...
# ============================================================================
# APLICACIÓN DASH CON ESTILO FLOURISH + NUEVA PESTAÑA DE TABLAS
# ============================================================================
...
app.layout = html.Div([
    ...
    dcc.Tabs(id='tabs', value='inasistencias', children=[
        ...
        dcc.Tab(label='📋 Explorador de Tablas', value='tablas'),  # NUEVA PESTAÑA
...
def render_content(tab):
    if tab == 'inasistencias':
        ...
        # NUEVA PESTAÑA: Explorador de tablas
        # Crear opciones para el dropdown
        table_options = [{'label': v['name'], 'value': k} for k, v in TABLE_QUERIES.items()]
        
        return html.Div([
            html.H2("Explorador de Tablas",
                   style={'marginBottom': '30px', 'color': FLOURISH_COLORS['text_primary']}),
            
            html.P("Selecciona hasta dos tablas para visualizar simultáneamente. Las tablas incluyen JOINs automáticos para mostrar información relacionada.", 
                  style={'color': FLOURISH_COLORS['text_secondary'], 'marginBottom': '20px'}),
            
            # TABLA 1
            html.Div([
                html.Label("📊 Tabla 1:", style={'fontWeight': 'bold', 'fontSize': '16px', 'marginBottom': '10px'}),
                dcc.Dropdown(
                    id='table-dropdown-1',
                ...
                html.Div(id='table-info-1', style={'marginBottom': '10px', 'color': FLOURISH_COLORS['text_secondary'], 'fontSize': '13px'}),
                html.Div(id='table-container-1', className='dash-table-container')
            ], className='table-selector', style={'marginBottom': '30px'}),
            
            # TABLA 2
            html.Div([
                html.Label("📊 Tabla 2:", style={'fontWeight': 'bold', 'fontSize': '16px', 'marginBottom': '10px'}),
                dcc.Dropdown(
                    id='table-dropdown-2',
                    options=table_options,
                    value='saldos',
                ...
                html.Div(id='table-info-2', style={'marginBottom': '10px', 'color': FLOURISH_COLORS['text_secondary'], 'fontSize': '13px'}),
                html.Div(id='table-container-2', className='dash-table-container')
...
def update_table_1(table_key):
    if not table_key:
        return html.Div("Selecciona una tabla"), ""
    
    try:
        df = get_table_data(table_key)
        
        if df.empty:
            return html.Div("⚠️ No hay datos en esta tabla", style={'color': FLOURISH_COLORS['yellow'], 'padding': '20px'}), ""
        ...
        table = dash_table.DataTable(
            ...
            style_header={
                ...
                'color': 'white',
                'fontWeight': '600',
                'border': '1px solid ' + FLOURISH_COLORS['purple']
            },
            style_data={
            ...
            style_data_conditional=[
                {
                    'if': {'row_index': 'odd'},
                    'backgroundColor': FLOURISH_COLORS['bg_secondary']
                },
                {
                    'if': {'state': 'active'},
                    'backgroundColor': FLOURISH_COLORS['purple_light'],
                    'border': '1px solid ' + FLOURISH_COLORS['purple']
                }
            ],
            page_size=20,
        ...
        return html.Div(f"❌ Error al cargar tabla: {str(e)}", style={'color': FLOURISH_COLORS['red'], 'padding': '20px'}), ""
    ...
    try:
        ...
        table = dash_table.DataTable(
            ...
            style_header={
                ...
                'color': 'white',
                'fontWeight': '600',
                'border': '1px solid ' + FLOURISH_COLORS['blue']
            },
            style_data={
                'backgroundColor': 'white',
                'color': FLOURISH_COLORS['text_primary']
            ...
            style_data_conditional=[
                ...
                {
                    'if': {'state': 'active'},
                    'backgroundColor': FLOURISH_COLORS['blue_light'],
                    'border': '1px solid ' + FLOURISH_COLORS['blue']
                }
            ],
            page_size=20,
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("  DASHBOARD RRHH - Centro Cultural (Estilo Flourish + Tablas)")
    print("="*70)
    ...
    print("\n   🆕 Nueva pestaña: Explorador de Tablas (con JOINs automáticos)")
