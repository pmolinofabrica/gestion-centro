-- ============================================================================
-- SISTEMA DE GESTIÓN DE RECURSOS HUMANOS - VERSION FINAL 2.0
...
CREATE TABLE turnos (
    ...
    turno_notas TEXT,
    activo BOOLEAN DEFAULT 1,
    CONSTRAINT chk_tipo_turno CHECK (tipo_turno IN ('mañana', 'tarde', 'capacitacion', 'apertura_publico', 'descanso')),
...
-- ============================================================================
-- MÓDULO 3: CONVOCATORIAS (Versión Mejorada - Inmutable)
-- ============================================================================
...
-- Trigger: Prevenir múltiples convocatorias vigentes para mismo agente/fecha
...
-- ============================================================================
-- MÓDULO 4: SISTEMA DE CAMBIOS DE TURNO (Modelo Transaccional)
-- ============================================================================
...
CREATE TABLE cambio_transaccion_detalle (
    ...
    validacion_capacitacion BOOLEAN DEFAULT 0,
    validacion_disponibilidad BOOLEAN DEFAULT 0,
    validacion_conflicto BOOLEAN DEFAULT 0,
...
CREATE TABLE cambio_validacion (
    ...
    es_bloqueante BOOLEAN DEFAULT 1,
    es_alerta BOOLEAN DEFAULT 0,
...
-- ============================================================================
-- MÓDULO 5: DESCANSOS Y DISPONIBILIDAD
-- ============================================================================
...
-- ============================================================================
-- MÓDULO 6: INASISTENCIAS Y CERTIFICADOS (Mejorado con Triggers)
-- ============================================================================
...
BEGIN
    UPDATE inasistencias SET
        requiere_certificado = CASE
            WHEN NEW.motivo IN ('medico', 'estudio', 'otro_justificada') THEN 1
            ELSE 0
        END,
        estado = CASE
            WHEN NEW.motivo IN ('medico', 'estudio', 'otro_justificada') THEN NEW.estado
            ELSE 'injustificada'
        END
...
AFTER UPDATE OF motivo ON inasistencias
FOR EACH ROW
WHEN OLD.motivo != NEW.motivo
BEGIN
    UPDATE inasistencias SET
        requiere_certificado = CASE
            WHEN NEW.motivo IN ('medico', 'estudio', 'otro_justificada') THEN 1
...
-- ============================================================================
-- MÓDULO 7: MENÚ (Asignación de Dispositivos)
-- ============================================================================
...
CREATE TABLE menu (
    ...
    orden INTEGER DEFAULT 1,
    acompaña_grupo BOOLEAN DEFAULT 0,
    fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- ============================================================================
-- MÓDULO 8: SALDOS (Con Triggers Automáticos COMPLETOS)
-- ============================================================================
...
-- Trigger 1: Insertar convocatoria → actualizar saldos
CREATE TRIGGER trg_saldo_insert_convocatoria
AFTER INSERT ON convocatoria
FOR EACH ROW
WHEN NEW.estado IN ('confirmada', 'vigente')
BEGIN
...
-- Trigger 2: UPDATE estado convocatoria → recalcular saldos
...
-- Trigger 3: DELETE convocatoria → recalcular saldos
...
-- ============================================================================
-- MÓDULO 9: SISTEMA DE LOGGING Y MONITOREO (COMPLETO)
-- ============================================================================
...
-- Trigger: Detectar patrones automáticamente (COMPLETO)
...
BEGIN
    ...
    ON CONFLICT(pattern_signature) DO UPDATE SET
        ...
        severity_max = CASE 
            ...
            WHEN NEW.severity = 'high' AND severity_max != 'critical' THEN 'high'
            WHEN NEW.severity = 'medium' AND severity_max NOT IN ('critical', 'high') THEN 'medium'
            ELSE severity_max
        END,
...
-- Trigger: Actualizar timestamp de resolución
...
-- ============================================================================
-- MÓDULO 10: TABLA DE CONFIGURACIÓN
-- ============================================================================
...
-- Vista 3: Ocupación de dispositivos
...
LEFT JOIN menu m ON disp.id_dispositivo = m.id_dispositivo
    AND m.fecha_asignacion >= date('now', '-30 days')
WHERE disp.activo = 1
...
SELECT 
    ...
    CASE 
        WHEN cap_part.aprobado = 1 THEN 'CAPACITADO'
        WHEN cap_part.asistio = 1 AND cap_part.aprobado IS NULL THEN 'EN EVALUACIÓN'
        WHEN cap_part.asistio = 1 AND cap_part.aprobado = 0 THEN 'NO APROBÓ'
        ELSE 'NO ASISTIÓ'
...
SELECT 
    ...
    CASE 
        WHEN inas.requiere_certificado = 1 THEN
...
WHERE strftime('%Y-%m', inas.fecha_inasistencia) = strftime('%Y-%m', 'now')
...
SELECT 
    ...
    e.user_action,
    CASE 
        WHEN e.is_recurring = 1 THEN '⚠️ RECURRENTE'
        ELSE ''
    END AS alerta
...
SELECT 
    ...
    ROUND((julianday('now') - julianday(p.first_occurrence)), 1) AS dias_activo,
    p.severity_max AS severidad_maxima,
    p.pattern_status AS estado,
    CASE 
        WHEN p.occurrence_count >= 10 THEN '🔴 CRÍTICO'
        WHEN p.occurrence_count >= 5 THEN '🟡 ATENCIÓN'
        ELSE '🟢 BAJO'
...
SELECT 
    (SELECT COUNT(*) 
     FROM system_errors 
     WHERE timestamp >= datetime('now', '-1 day')) AS errores_24h,
    
    (SELECT COUNT(*) 
     FROM system_errors 
     WHERE severity = 'critical' 
     AND resolved = 0) AS criticos_pendientes,
    
    (SELECT COUNT(*) 
     FROM system_errors 
     WHERE timestamp >= datetime('now', '-7 days')) AS errores_semana,
    ...
    ROUND(
        CAST((SELECT COUNT(*) FROM system_errors WHERE resolved = 1) AS FLOAT) /
        NULLIF((SELECT COUNT(*) FROM system_errors), 0) * 100,
        2
    ) AS tasa_resolucion_porcentaje,
    
    CASE 
        WHEN (SELECT COUNT(*) FROM system_errors WHERE severity = 'critical' AND resolved = 0) > 0 
            THEN '🔴 CRÍTICO'
        WHEN (SELECT COUNT(*) FROM system_errors WHERE timestamp >= datetime('now', '-1 day')) > 10 
            THEN '🟡 ATENCIÓN'
    ...
    datetime('now') AS fecha_reporte;

-- Vista 10: Análisis por componente
...
SELECT 
    ...
    SUM(CASE WHEN resolved = 1 THEN 1 ELSE 0 END) AS resueltos,
...
/*
SISTEMA DE GESTIÓN DE RRHH - VERSIÓN FINAL 2.0
===============================================
...
2. Capacitaciones (3 tablas) ← INCLUYE capacitaciones_participantes
...
7. Menú/Asignación Dispositivos (1 tabla)
8. Saldos (1 tabla) ← TRIGGERS COMPLETOS
9. Sistema de Logging (2 tablas)
10. Configuración (1 tabla)
...
3. trg_registrar_historial_cambio - Historial automático
4. trg_asignar_descanso_aprobado - Auto-asignación de descansos
5. trg_auto_requiere_certificado - Lógica de certificados
6. trg_update_requiere_certificado - Actualización certificados
7. trg_certificado_aprobado - Workflow aprobación
...
12. trg_detectar_patron_error - Detección de patrones
13. trg_error_resuelto - Timestamps de resolución
...
10. vista_errores_por_componente - Análisis por componente
11. vista_errores_timeline - Timeline para gráficos
...
MANTENIMIENTO:
-------------
- Limpiar errores resueltos: >90 días
